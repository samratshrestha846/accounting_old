...
Invoice.prototype = {
    ...
    getdata: function () {
        jQuery($.opt.parentClass).each(function (i){
            ...
            var countselectedserial = 0;
            var countpreselectedserial = 0;
            if(row.find($.opt.serialcount).val() > 0){
                ...
                if(typeof(selectedserial) != 'undefined'){
                    ...
                    countselectedserial = Object.keys(selectedserial).length;
                    // row.find($.opt.qty).val(countselectedserial);
                }
            }
            if(row.find($.opt.preserialcount).val() > 0){
                var preselectedserial = row.find($.opt.preqtyoption).val();
                if(typeof(preselectedserial) != 'undefined'){
                    // var arrselectedserial = Object.values(selectedserial);
                    // console.log(typeof(arrselectedserial));
                    countpreselectedserial = Object.keys(preselectedserial).length;
                    // row.find($.opt.qty).val(countselectedserial);
                }
            }
            if(countselectedserial + countpreselectedserial > 0){
                row.find($.opt.qty).val(countselectedserial + countpreselectedserial);
            }
            ...
            if(Number(row.find($.opt.stock).val()) < Number(row.find($.opt.qty).val())){
        ...
        //         if(typeof(row.find($.opt.item).find(':selected').val()) != 'undefined'){
        ...
        //                         if(warehouseproducts[z].product_id == row.find($.opt.item).find(':selected').val()){
        ...
        //                                     if(allserialnumbersforedit[s].billing_id == window.billing_id){
        ...
        //                                         </option>`;
        //                                     }
        //                                     row.find($.opt.preserial_numbers).html(selectedserialoptions);
        //                                 }

        //                             }
        //                             row.find($.opt.preserialcount).val(allserialnumbersforeditcount);
    ...
    calcindTax: function() {
        ...
        jQuery($.opt.parentClass).each(function (i) {
            ...
            var txamt = rate * selectedtaxper/100;
    ...
    calcGrandTotal: function () {
        ...
        if(typeof window.att == "undefined"){
            ...
            if(jQuery($.opt.payment_type).find(':selected').val() == 'paid'){
                jQuery($.opt.payment_amount).val(grandTotal);
            }
        }
    ...
    $(function() {
        ...
        <td style="${selected_filter_array.includes('unit') == false ? 'display:none' : ''}"><input class="form-control unit" placeholder="Unit" type="text" name="unit[]" readonly></td>
        <td style="${selected_filter_array.includes('rate') == false ? 'display:none' : ''}"><input class="form-control rate" placeholder="Rate" type="text" name="rate[]"></td>
